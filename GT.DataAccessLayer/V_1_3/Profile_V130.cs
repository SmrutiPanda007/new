...
{
    public class Profile_V130 : DataAccess
    {
        public Profile_V130(string sConnString)
            : base(sConnString)
        {
        ...
        public int PhoneContactsSync(DataTable phoneContacts, DataTable editContacts, string deleteContacts, string deviceUniqueID, string deviceName, int userID, int isFirstTime, out string retMsg)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet UpdateProfile(int userID, string nickName, string emailID, string workNumber, string webSiteURL, string company, out int retVal, out string retMsg, out int errorCode)
        {
            SqlConnection sqlCon = Connection; //System.Configuration.ConfigurationManager.ConnectionStrings("smscconferenceconnectionstring"));
            SqlCommand sqlCmd = default(SqlCommand);
            DataSet ds = new DataSet();
