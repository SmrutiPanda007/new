...
{
    public class Groups_V130 : DataAccess
    {
        public Groups_V130(string sConnString)
            : base(sConnString)
        {
        ...
        public void MemberLeaveFromGrpCall(int userId, int grpTalkID, out short leaveStatus, out string grpTalkName, out string deviceToken, out short osID, out string hostMobile, out string schTime, out short retVal, out string retMessage, out string leaveMemName,out int errorCode)
        {
        ...
        public DataSet EditGroupCall(DataTable grpCallMembers, int userId, grpEdit grpObj, out short retVal, out string retMessage,out int errorCode)
        {
            SqlCommand sqlCmd = default(SqlCommand);
            SqlConnection sqlCon = null;
            SqlDataAdapter da = null;
            ...
            try
            {
                ...
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.Parameters.Add("@UserId", SqlDbType.BigInt).Value = userId;
                sqlCmd.Parameters.Add("@IsFromApp", SqlDbType.TinyInt).Value = 1;
        ...
        public DataSet SaveContactsToConference(DataTable members, int userID, int confID,string webListIds, out int retVal, out string retMessage,out int errorCode)
        {
            ...
            try
            {
                sqlCmd = new SqlCommand("GT_AddParticipantInGroupCall", sqlCon);
                sqlCmd.CommandType = CommandType.StoredProcedure;
                sqlCmd.Parameters.Add("@userid", SqlDbType.Int).Value = userID;
