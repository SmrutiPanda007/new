...
namespace GT.DataAccessLayer.V_1_5
{
    ...
    public class Profile : DataAccess
    {
        public Profile(string sConnString)
            : base(sConnString)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet UpdateProfile(int userID, string nickName, string emailID, string offSet, string company, out int retVal, out string retMsg, out int errorCode)
        {
            SqlConnection sqlCon = Connection; //System.Configuration.ConfigurationManager.ConnectionStrings("smscconferenceconnectionstring"));
            SqlCommand sqlCmd = default(SqlCommand);
            DataSet ds = new DataSet();
