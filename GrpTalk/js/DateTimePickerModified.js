...
; (function ($, window, document, undefined) {
    ...
    DateTimePicker.prototype = {
        ...
        _setOutput: function () {
            ...
            datetimetotaltext = sOutput.split(' ')
            ...
            var dateparam = datetotaltext[1] + '-' + datetotaltext[0] + '-' + datetotaltext[2]
            $("#ui_date_picker").val(dateparam);
            $("#from_time").val(outtimetext);
            return outdatetext;
        ...
        _showPicker: function (sMode, sMinValue, sMaxValue, sFormat, sCurrent, oElement, sStartEnd, sStartEndElem) {
            ...
            if (dtPickerObj._compare(dtPickerObj.settings.mode, "date")) {
                ...
                if (sMin != "" && sMin != null)
                    ...
                    dtPickerObj.dataObject.dMinValue = dtPickerObj._parseDateTime(sMin, 1);
        ...
        _modifyPicker: function () {
            ...
            //sDTPickerButtons += "</div><div align='center' class='margin-top-10' style='font-size:15px;'><a href='#' class='dlater'>Decide Later</a></div>";
        ...
        _parseDateTime: function (sDateTime, type) {
            var dtPickerObj = this;

            var dateSplit = new Array();
            if (sDateTime != "") {
                dateSplit = sDateTime.split("at");
                if (dateSplit.length == 2)
                { sDateTime = ""; sDateTime = myDateFormatter(dateSplit[0] + " " + dateSplit[1]); }
            }
            else if (sDateTime == "") {
                var newDate = getGrpTalkUserDateTime();
                sDateTime = myDateFormatter(newDate);

            }

            if (plugoutput != "") { sDateTime = plugoutput }
            if (type != "" && type == 1) { sDateTime = dtPickerObj.settings.minDateTime }

            var dTempDate = new Date(dtPickerObj.settings.defaultDate);

            var iDate, iMonth, iYear, iHour, iMinutes, sMeridiem;
