...
{
    public class GroupCallReports_V120
    {
        string grpCallName = "";
        ...
        public DataSet UpdateCallBacksCallDb(string sConnString, CallBackVariable callBackProp, string callBackAddr)
        {
            ...
            DataAccessLayer.V_1_2.GroupcallReports_V120 groupCallReportsObj = new DataAccessLayer.V_1_2.GroupcallReports_V120(sConnString);
        ...
        /// <returns></returns>
        public JObject GetReportsByBatchIdNew(string sConnString, int userID, int confID, string batchID)
        {
            DataSet ds = new DataSet();
            JObject jObj = new JObject();
            JObject jObj1 = new JObject();
            int retVal = 0, isConfRecorded = 0;
            // int isRecorded = 0;
            string retMessage = "";
            ...
            {
                DataAccessLayer.V_1_2.GroupcallReports_V120 groupCallReportsObj = new DataAccessLayer.V_1_2.GroupcallReports_V120(sConnString);
                ds = groupCallReportsObj.GetReportsByBatchIdNew(userID, confID, batchID, out retVal, out retMessage, out isCallFromBonus);
                if (retVal == 1)
                {
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        if (ds.Tables[2].Rows.Count > 0)
                        {
                            ...
                            for (int iterator = 0; iterator < _with3.Rows.Count; iterator++)
                            {
                                ...
                                jArr1.Add(new JObject(new JProperty("Member", _with3.Rows[iterator]["number"]),
