...
namespace GT.BusinessLogicLayer.V_1_3
{
    ...
    public class Contacts_V130
    {
        ...
        public JObject CropImage(string sConnString, int userID, string name, string mobileNumber, string prefix, int contactID, int mode, int listID, string saveaPath, string readPath, string imgName, int w, int h, int x, int y, int fixWidth)
        {
                ...
                if (readPath != "")
                {
                    ...
                    using (MemoryStream ms = new MemoryStream(CropImage, 0, CropImage.Length))
                    {
                        ...
                        using (SD.Image CroppedImage = SD.Image.FromStream(ms, true))
                        {
                            ...
                            jres = ManageWebContacts(sConnString, userID, name, mobileNumber, prefix, contactID, mode, listID, imgName);
        ...
        public JObject ManageWebContacts(string sConnString, int userId, string name, string mobileNumber, string prefix, int contactID, int mode, int listID, string tempCropImagePath)
        {
            JObject responseJObj = new JObject();
            ...
            try
            {
                ...
                DataSet ds = contactsObj.ManageWebContacts(userId, name, mobileNumber, prefix, contactID, mode, listID, out returnValue, out returnMessage, tempCropImagePath,out contactId);
                if (returnValue == 1)
                {
                    responseJObj = new JObject(new JProperty("Success", true),
        ...
        public JObject UploadExcelData(string sConnString, int userID, DataTable dt,int listId)
        {
            JObject responseJObj = new JObject();
            JArray jar = new JArray();
            int returnValue = 0;
            ...
            try
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        Logger.TraceLog(dc.ColumnName + "---" + dr[dc.ColumnName]);
                    }
                }
                DataAccessLayer.V_1_3.Contacts_V130 contactsObj = new DataAccessLayer.V_1_3.Contacts_V130(sConnString);
                ds = contactsObj.UploadExcelData(userID, dt, out returnValue, out returnMessage,listId);
                if (returnValue == 1)
                {
                    responseJObj = new JObject(new JProperty("Success", true),
                         new JProperty("Status", returnValue), new JProperty("ErrorCode", "117"),
