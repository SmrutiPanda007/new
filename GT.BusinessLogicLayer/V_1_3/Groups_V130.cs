    ...
    public class Groups_V130
    {
        ...
        public JObject MemberLeaveFromGrpCall(string sConnString, int ConferenceID, int userId)
        {
            ...
            try
            {
                ...
                string leaveMemName = "";
                int errorCode = 0;
                DataAccessLayer.V_1_3.Groups_V130 leaveGroupObj = new DataAccessLayer.V_1_3.Groups_V130(sConnString);
                leaveGroupObj.MemberLeaveFromGrpCall(userId, grpTalkID, out leaveStatus, out grpTalkName, out deviceToken, out osID, out hostMobile, out schTime, out retVal, out retMessage, out leaveMemName, out errorCode);
                if (retVal == 0)
                {
        ...
        public JObject CreateGroupCall(string sConnString, JObject paramObj, int userId)
        {
            ...
            else
            {
                foreach (JObject _Member in (JArray)paramObj.SelectToken("Participants"))
                {
                    ...
                    MembersForThisGroup.Rows.Add(MemberName, MemberMobile, 0, DndFlag);
                }
            }
            ...
            try
            {
                ...
                if (retVal == 1)
                {
                    if (paramObj.SelectToken("WebListIds").ToString() != "")
                    {
                        foreach (DataRow _row in grpCallInfo.Tables[1].Rows)
                        {
                            weblistJobj = new JObject();
                            foreach (DataColumn _column in grpCallInfo.Tables[1].Columns)
                            {
        ...
        public JObject EditGroupCall(string sConnString, JObject paramObj, int userId)
        {
            ...
            else
            {
                foreach (JObject _Member in (JArray)paramObj.SelectToken("Participants"))
                {
                    ...
                    membersForThisGroupCall.Rows.Add(MemberName, MemberMobile, 0, DndFlag);
                }
            }
            ...
            try
            {
                grpCallInfo = groupsObj.EditGroupCall(grpCallMembers, userId, editObj, out retVal, out RetMessage, out errorCode);
                ...
                try
                {
                    ...
                    else
                    {
                        foreach (JObject _Members in (JArray)paramObj.SelectToken("Participants"))
                        {
                            ...
                            membersForThisConference.Rows.Add(MemberName, MemberMobile, 0, DndFlag);
