    ...
    public class Profile_V130
    {
        ...
        public JObject GetProfileDetailsDisplay(string sConnString, int userId, int mode)
        {
            ...
            try
            {
                int retVal = 0;
                string retMessage = "";
                DataAccessLayer.V_1_3.Profile_V130 profileObj = new DataAccessLayer.V_1_3.Profile_V130(sConnString);
        ...
        public JObject PhoneContactsSync(string sConnString, JObject paramObj, int UserID, string deviceUniqueID, string deviceName)
        {
            ...
            try
            {
                foreach (JObject _Member in (JArray)paramObj.SelectToken("Contacts"))
                {
                    ...
                    phoneContacts.Rows.Add(name, mobileNumbers, mobileTypes, deviceContactID, contactImagePath);
                }
            ...
            {
                DataAccessLayer.V_1_3.Profile_V130 profileObj = new DataAccessLayer.V_1_3.Profile_V130(sConnString);
                retVal = profileObj.PhoneContactsSync(phoneContacts, editContacts, deleteContacts, deviceUniqueID, deviceName, UserID, Convert.ToInt32(paramObj.SelectToken("IsFirstTime")), out retMsg);
                if (retVal == 1)
                {
            ...
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    jObj = new JObject(new JProperty("Status", true),
                                     ...
                                     new JProperty("imagepath", ConfigurationManager.AppSettings["WebUrl"].ToString() + "TempImages/" + tempFileName.ToString() + ""));
                }
                else
                {
        ...
        public JObject UpdateProfile(string sConnString, int userID, string nickName, string emailID, string workNumber, string webSiteURL, string company)
        {
            ...
            {
                DataAccessLayer.V_1_3.Profile_V130 profileObj = new DataAccessLayer.V_1_3.Profile_V130(sConnString);
                ds = profileObj.UpdateProfile(userID, nickName, emailID, workNumber, webSiteURL, company, out retVal, out retMsg, out errorCode);
                if (retVal == 1)
                {
