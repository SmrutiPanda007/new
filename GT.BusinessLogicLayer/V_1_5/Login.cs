...
namespace GT.BusinessLogicLayer.V_1_5
{
    public class Login
    {
        ...
        public JObject OtpCall(string sConnString, string mobile)
        {
            ...
            DataAccessLayer.V_1_5.Login loginObj = new DataAccessLayer.V_1_5.Login(sConnString);
            try
            {
        ...
        public bool ApiOtpCall(string MobileNumber, string Otp, string fromNumber)
        {
            ...
            new JProperty("RingUrl", "www.grtalk.com"),
            new JProperty("AnswerUrl", "www.grtalk.com"),
            new JProperty("HangupUrl", "www.grtalk.com"),
            new JProperty("HttpMethod", "POST"),
            new JProperty("Xml", xml));
            Logger.TraceLog("OTP CALL" + requestJobj.ToString());
        ...
        public JObject ValidateOTP(string sConnString, string mobile, string OTP, string txnID, int isRegisteredFromWeb, int countryID)
        {
            DataAccessLayer.V_1_5.Login loginObj = new DataAccessLayer.V_1_5.Login(sConnString);
            DataSet ds = new DataSet();
            JObject jObj;
            ...
            try
            {
                ...
                if (retVal == 1)
                {
                    ...
                    jObj.Add(new JProperty("MinimumBalance", minimumBalance));
                    jObj.Add(new JProperty("MinMemberCount", 1));
        ...
        public JObject Registration(string sConnString, JObject paramObj, string clientIpAddr, string txnID, Boolean isResend, int countryID)
        {
            ...
            string senderIds = ConfigurationManager.AppSettings["senderId"].ToString();
            ...
            try
            {
                ...
                if (detailsObj.OsID != 3)
                { detailsObj.UserName = paramObj.SelectToken("UserName").ToString(); }
                if (isResend)
                {
                ...
                if (retVal == 1)
                {
                    jObj = new JObject(new JProperty("Success", true),
                            ...
                            new JProperty("SenderIds", senderIds),
                            new JProperty("Message", retMessage), new JProperty("ErrorCode", errorCode));
                }
                else
                {
        ...
        public JObject QR_Code_Check(string sConnString, JObject jObj, int userID)
        {
            ...
            Logger.TraceLog(pusherAppId + pusherAppKey + pusherAppsecret);
            DataAccessLayer.V_1_5.Login loginObj = new DataAccessLayer.V_1_5.Login(sConnString);
            ...
            if (retVal == 1)
            {
                ...
                Logger.TraceLog("PusherResponse" + PusherResponse.Body);
                ...
                if (retVal == 1)
                {
                    if (ds.Tables.Count != 0)
                    {
                        if (ds.Tables[0].Rows.Count != 0)
                        {
                            ...
                            {
                                //Logger.ExceptionLog("Entered here mygrps.aspx" + ds.Tables[0].Rows[0]["UserType"].ToString());
                                jresponseObj.Add(new JProperty("RedirectURL", ConfigurationManager.AppSettings["WebUrl"] + "mygroup.aspx"));
                            }
                            else
                            {
                                //Logger.ExceptionLog("Entered here mygrptalks.aspx" + ds.Tables[0].Rows[0]["UserType"].ToString());
                                jresponseObj.Add(new JProperty("RedirectURL", ConfigurationManager.AppSettings["WebUrl"] + "mygrptalks.aspx"));
                            }
        ...
        public JObject QrCodeLogout(JObject jobj, string sConnString, int userId)
        {
            ...
            try
            {
                ...
                {
                    GT.DataAccessLayer.V_1_5.Login loginObj = new DataAccessLayer.V_1_5.Login(sConnString);
