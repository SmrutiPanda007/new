...
namespace GT.BusinessLogicLayer
{
    ...
    class PlivoClientBusiness
    {
        ...
        public JObject MakeCall(string from, string to)
        {
            ...
            {
                responseObj = new JObject(new JProperty("Success", false), new JProperty("Message", "Server Internal Error"), new JProperty("ErrorCode", "101"));
        ...
        public JObject MakeBulkCall(string from, string to, string delimiter)
        {
            ...
            try
            {
                Logger.TraceLog("Make Bulk call for bahrain jain gateway"+from);
                parameters.Add("from", from);
                parameters.Add("answer_url", answerUrl);
                parameters.Add("ring_url", ringUrl);
                parameters.Add("answer_method", "GET");
        ...
        public JObject HangupCalls(string requestUUIDs)
        {
            ...
            try
            {
                ...
                Logger.TraceLog("HangupCalls Parameter" + parameters.ToString());
                restResp = clientObj.hangup_call(parameters);
                Logger.TraceLog("HangupCalls Rest Reponse" + restResp.ToString());
                if (restResp.StatusCode == System.Net.HttpStatusCode.NoContent)
                {
                    responseObj = new JObject(new JProperty("Success", true), new JProperty("Message", "Hangup Success"), new JProperty("ErrorCode", "137"));
                }
                else
                {
                    responseObj = new JObject(new JProperty("Success", false), new JProperty("Message", restResp.Data.error));
        ...
        public JObject ConferenceMute(string conferenceName, string memberIDs)
        {
            ...
            try
            {
                ...
                {
                    responseObj = new JObject(new JProperty("Success", true), new JProperty("Message", "OK"), new JProperty("ErrorCode", "117"));
                }
                else
                {
                    responseObj = new JObject(new JProperty("Success", false), new JProperty("Message", "Error In Mute"), new JProperty("ErrorCode", "141"));
                }
            }
            catch (Exception ex)
            {
        ...
        public JObject ConferenceUnMute(string conferenceName, string memberIDs)
        {
            ...
            try
            {
                ...
                {
                    responseObj = new JObject(new JProperty("Success", false), new JProperty("Message", "Error In UnMute"), new JProperty("ErrorCode", "141"));
                }
            }
            catch (Exception ex)
            {
                responseObj = new JObject(new JProperty("Satus", 0), new JProperty("Message", "Server Internal Error"), new JProperty("ErrorCode", "101"));
                Logger.ExceptionLog("Exception in PlivoClientBusiness is " + ex.ToString());
            }
            return responseObj;
        }
        ...
        public JObject KillConference(string conferenceName)
        {
            ...
            try
            {
                ...
                if (restResp.StatusCode == System.Net.HttpStatusCode.NoContent)
                {
                    responseObj = new JObject(new JProperty("Success", true), new JProperty("Message", "Conference Hungup"), new JProperty("ErrorCode", "137"));
                }
                else
                {
                    responseObj = new JObject(new JProperty("Success", false), new JProperty("Message", restResp.Data.error));
